...
class roberta_en_context(nn.Module):
    ...
    def __init__(self):        
        super().__init__() 
        self.roberta_model = AutoModel.from_pretrained('/root/qxy/MMML/models/roberta-base',local_files_only=True) # with context, we can improve using a larger model
...
class rob_d2v_cc_context(nn.Module):            
    def __init__(self, config):        
        super().__init__()
        self.roberta_model = RobertaModel.from_pretrained('/root/qxy/MMML/models/roberta-base',local_files_only=True)
        #self.data2vec_model = Data2VecAudioModel.from_pretrained("facebook/data2vec-audio-base")
        self.hubert_model = HubertModel.from_pretrained('/root/qxy/MMML/models/TencentGameMate/chinese-hubert-base',local_files_only=True)
...
class rob_d2v_cme_context(nn.Module):            
    def __init__(self, config):        
        ...
        # CME layers
        Bert_config = BertConfig(num_hidden_layers=config.num_hidden_layers, hidden_size=768, intermediate_size=3072, num_attention_heads=12)
